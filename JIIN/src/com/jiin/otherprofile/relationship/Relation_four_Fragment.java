...
public class Relation_four_Fragment extends Fragment {
	
	ArrayList<User> relation = new ArrayList<User>();
	
	ImageView me, friend1, friend2, friend3, you;
	View meFrame, youFrame, bar;
	
	@Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		final View view= inflater.inflate(R.layout.fragment_relation_4, container, false);
		...
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(3).mainPicture, friend1);
				new Handler().postDelayed(new Runnable() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(2).mainPicture, friend2);
				new Handler().postDelayed(new Runnable() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(1).mainPicture, friend3);
				new Handler().postDelayed(new Runnable() {
