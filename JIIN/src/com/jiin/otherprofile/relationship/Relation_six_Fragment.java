...
public class Relation_six_Fragment extends Fragment {
	
	ArrayList<User> relation = new ArrayList<User>();
	
	ImageView me, friend1, friend2, friend3, friend4, friend5, you;
	View meFrame, youFrame, bar;
	
	@Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		final View view= inflater.inflate(R.layout.fragment_relation_6, container, false);
		...
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(5).mainPicture, friend1);
				new Handler().postDelayed(new Runnable() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(4).mainPicture, friend2);
				new Handler().postDelayed(new Runnable() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(3).mainPicture, friend3);
				new Handler().postDelayed(new Runnable() {
				...
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(2).mainPicture, friend4);
				new Handler().postDelayed(new Runnable() {
				...
				friend5 = (ImageView)view.findViewById(R.id.image_friend5);
				ImageLoader.getInstance().displayImage(JiinConstant.SERVER_URL+relation.get(1).mainPicture, friend5);
				new Handler().postDelayed(new Runnable() {
					
					@Override
					public void run() {
						friend5.setVisibility(View.VISIBLE);
						friend5.setAnimation(AnimationUtils.loadAnimation(getActivity(), R.anim.relation_popup));
					}
				...
				}, JiinConstant.RELATION_DELAY_TIME*8);
